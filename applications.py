...
class Application(tk.Frame):
	...
	def draw_title_widgets(self):
		...
		self.title = tk.Label(self.title_frame, width=24, height=2,
					text="Rounik's Bristo Cafe", font=('verdana',22,'bold'), fg="#248aa2", bg="white")
	...
	def get_receipt(self):
		if self.chargeValues[4].get() != '':
			...
			if name:
				...
				messagebox.showinfo("Rounik's Cafe", 'Receipt Generated')
			else:
				messagebox.showerror("Rounik's Cafe", 'Customer Name is required.')
		else:
			messagebox.showerror("Rounik's Cafe", 'No items Bought')
